  ...
  try {
    ...
    const { data: planDesign, error: fetchErr } = await admin
      ...
      .select(`
        ...
        groups(id, name, member_count, location, industry)
      `)
      .eq('id', designId)
      .maybeSingle();
    ...
    // CHANGED S42: client → group. The downstream PDF code still receives an
    // object with employer_name to minimize the rewrite; we adapt the group fields.
    const groupRow: any = Array.isArray(planDesign.groups)
      ? planDesign.groups[0]
      : planDesign.groups;
    const client = groupRow
      ? {
          id: groupRow.id,
          employer_name: groupRow.name,
          member_count: groupRow.member_count,
...
const styles = StyleSheet.create({
  page: {
    ...
    fontSize: 10, fontFamily: 'Helvetica', color: '#1e3a5f',
  },
  coverContent: { flex: 1, justifyContent: 'center', alignItems: 'center', textAlign: 'center' },
  coverBrand: { fontSize: 22, fontWeight: 'bold', color: '#1e3a5f', marginBottom: 6 },
  coverBrandAccent: { color: '#7a9b76', fontStyle: 'italic' },
  coverDivider: { width: 60, height: 2, backgroundColor: '#7a9b76', marginVertical: 24 },
  coverTitle: { fontSize: 28, fontWeight: 'bold', color: '#1e3a5f', marginBottom: 8, maxWidth: 400 },
  coverSubtitle: { fontSize: 14, color: '#3a4d68', marginBottom: 30 },
  coverMeta: { fontSize: 11, color: '#3a4d68', marginBottom: 4 },
  coverMetaLabel: { fontSize: 9, color: '#94a3b8', textTransform: 'uppercase', letterSpacing: 1 },
  coverFooter: { position: 'absolute', bottom: 40, left: 40, right: 40, textAlign: 'center', fontSize: 9, color: '#94a3b8' },
  sectionHeader: { flexDirection: 'row', alignItems: 'center', marginBottom: 16, paddingBottom: 8, borderBottomWidth: 2, borderBottomColor: '#7a9b76' },
  sectionHeaderNum: { fontSize: 18, fontWeight: 'bold', color: '#7a9b76', marginRight: 10 },
  sectionHeaderTitle: { fontSize: 18, fontWeight: 'bold', color: '#1e3a5f', flex: 1 },
  subSection: { marginBottom: 14 },
  subSectionTitle: { fontSize: 12, fontWeight: 'bold', color: '#1e3a5f', marginBottom: 6 },
  row: { flexDirection: 'row', paddingVertical: 4, borderBottomWidth: 0.5, borderBottomColor: '#e2e8f0' },
  rowLast: { borderBottomWidth: 0 },
  rowLabel: { width: 160, fontSize: 10, color: '#94a3b8', fontWeight: 'normal' },
  rowValue: { flex: 1, fontSize: 10, color: '#1e3a5f', fontWeight: 'bold' },
  summaryGrid: { flexDirection: 'row', flexWrap: 'wrap', gap: 8, marginBottom: 16 },
  summaryCard: { flexBasis: '48%', backgroundColor: '#faf7f2', borderRadius: 6, padding: 12, borderWidth: 0.5, borderColor: '#e2e8f0' },
  summaryCardHighlight: { backgroundColor: '#1e3a5f' },
  summaryCardLabel: { fontSize: 8, color: '#7a9b76', fontWeight: 'bold', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 4 },
  summaryCardLabelHighlight: { color: '#a8c4a4' },
  summaryCardValue: { fontSize: 18, fontWeight: 'bold', color: '#1e3a5f' },
  summaryCardValueHighlight: { color: '#ffffff' },
  summaryCardSub: { fontSize: 8, color: '#94a3b8', marginTop: 2 },
  summaryCardSubHighlight: { color: '#cbd5e0' },
  callout: { backgroundColor: '#f0f7fa', borderRadius: 6, padding: 10, borderWidth: 0.5, borderColor: '#bae6e6', marginBottom: 14 },
  calloutWarning: { backgroundColor: '#fffbeb', borderColor: '#fde68a' },
  calloutSuccess: { backgroundColor: '#f0fdf4', borderColor: '#bbf7d0' },
  calloutText: { fontSize: 9, color: '#0e7490', lineHeight: 1.5 },
  calloutTextWarning: { color: '#92400e' },
  calloutTextSuccess: { color: '#166534' },
  bulletItem: { flexDirection: 'row', marginBottom: 4, paddingLeft: 4 },
  bulletDot: { width: 10, fontSize: 10, color: '#7a9b76' },
  bulletText: { flex: 1, fontSize: 9.5, color: '#3a4d68', lineHeight: 1.5 },
  recommendation: { backgroundColor: '#f0fdf4', borderRadius: 6, padding: 10, marginBottom: 8, borderWidth: 0.5, borderColor: '#bbf7d0' },
  recommendationHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 4 },
  recommendationTitle: { fontSize: 10, fontWeight: 'bold', color: '#166534', flex: 1 },
  recommendationImpact: { fontSize: 9, color: '#7a9b76', fontWeight: 'bold', marginLeft: 8 },
  recommendationRationale: { fontSize: 9, color: '#3a4d68', lineHeight: 1.5 },
  disclaimer: { backgroundColor: '#faf7f2', borderRadius: 4, padding: 10, marginTop: 16, borderLeftWidth: 3, borderLeftColor: '#94a3b8' },
  disclaimerText: { fontSize: 8.5, color: '#3a4d68', fontStyle: 'italic', lineHeight: 1.5 },
  pageFooter: { position: 'absolute', bottom: 30, left: 40, right: 40, flexDirection: 'row', justifyContent: 'space-between', fontSize: 8, color: '#94a3b8', borderTopWidth: 0.5, borderTopColor: '#e2e8f0', paddingTop: 8 },
  bundledNotice: { backgroundColor: '#f8fafc', borderRadius: 4, padding: 8, marginBottom: 8, borderWidth: 0.5, borderColor: '#e2e8f0' },
  bundledNoticeText: { fontSize: 9, color: '#3a4d68', fontStyle: 'italic' },
});

function buildPdfDocument({
  planDesign, client, brokerName, agencyName,
}: {
  planDesign: any; client: any; brokerName: string; agencyName: string;
}) {
  ...
  // CHANGED S42: simpler since groups has a single `name` field
  const employerName = client?.employer_name || 'Unknown';
  ...
  return React.createElement(
    ...
    React.createElement(
      ...
      React.createElement(View, { style: styles.coverContent },
        React.createElement(Text, { style: styles.coverBrand },
          'Clarity ', React.createElement(Text, { style: styles.coverBrandAccent }, 'Health')),
        ...
        React.createElement(View, { style: { marginTop: 30 } },
          ...
          React.createElement(Text, { style: combine(styles.coverMeta, { marginBottom: 14 }) }, isSelfFunded ? 'Self-funded' : 'Level-funded'),
    ...
    projection ? React.createElement(Page, { size: 'LETTER', style: styles.page, key: 'summary' },
      sectionHeader('1', 'Executive summary'),
      projection.summary?.headline ? React.createElement(Text,
        { style: { fontSize: 12, color: '#1e3a5f', marginBottom: 14, fontStyle: 'italic' } },
        projection.summary.headline) : null,
      React.createElement(View, { style: styles.summaryGrid },
        ...
        summaryCardEl('Best case', fmtMoney(projection.summary?.totalAnnualCostBest), '~10-15% under expected'),
        summaryCardEl('Worst case', fmtMoney(projection.summary?.totalAnnualCostWorst), 'bad-claims year'),
        summaryCardEl('Max liability', fmtMoney(projection.maxLiability?.amount), 'if aggregate stop-loss hits'),
      ),
      projection.summary?.comparedToFullyInsured ? React.createElement(View, { style: styles.callout },
        React.createElement(Text, { style: styles.calloutText },
          React.createElement(Text, { style: { fontWeight: 'bold' } }, 'vs. fully-insured: '),
          projection.summary.comparedToFullyInsured)) : null,
    ...
    React.createElement(Page, { size: 'LETTER', style: styles.page, key: 'design1' },
      sectionHeader('2', 'Plan design'),
      React.createElement(View, { style: styles.subSection },
    ...
    React.createElement(Page, { size: 'LETTER', style: styles.page, key: 'design2' },
      ...
      React.createElement(View, { style: styles.subSection },
        ...
        isSelfFunded
          ...
          : React.createElement(View, { style: styles.bundledNotice },
    ...
    React.createElement(Page, { size: 'LETTER', style: styles.page, key: 'design3' },
    ...
    projection ? React.createElement(Page, { size: 'LETTER', style: styles.page, key: 'projection' },
      sectionHeader('5', 'AI cost projection'),
      projection.confidenceLevel ? React.createElement(View,
        { style: combine(styles.callout, projection.confidenceLevel === 'low' ? styles.calloutWarning : null) },
        React.createElement(Text,
          { style: combine(styles.calloutText, projection.confidenceLevel === 'low' ? styles.calloutTextWarning : null) },
          React.createElement(Text, { style: { fontWeight: 'bold' } }, `${cap(projection.confidenceLevel)} confidence: `),
          projection.confidenceExplanation || '')) : null,
      projection.maxLiability?.explanation ? React.createElement(View, { style: styles.subSection },
        ...
        React.createElement(Text, { style: { fontSize: 9.5, color: '#3a4d68', lineHeight: 1.5 } },
          projection.maxLiability.explanation)) : null,
      (projection.assumptions || []).length > 0 ? React.createElement(View, { style: styles.subSection },
        ...
        ...projection.assumptions.map((a: string, i: number) =>
          React.createElement(View, { key: i, style: styles.bulletItem },
            ...
            React.createElement(Text, { style: styles.bulletText }, a)))) : null,
      (projection.sensitivityFlags || []).length > 0 ? React.createElement(View, { style: styles.subSection },
        ...
        ...projection.sensitivityFlags.map((s: any, i: number) =>
          React.createElement(View, { key: i, style: { marginBottom: 6 } },
            ...
            React.createElement(Text, { style: { fontSize: 9, color: '#3a4d68', lineHeight: 1.5 } }, s.impact)))) : null,
      (projection.recommendations || []).length > 0 ? React.createElement(View, { style: styles.subSection },
        ...
        ...projection.recommendations.map((r: any, i: number) =>
          React.createElement(View, { key: i, style: styles.recommendation },
            React.createElement(View, { style: styles.recommendationHeader },
            ...
            React.createElement(Text, { style: styles.recommendationRationale }, r.rationale)))) : null,
      pageFooter(footerText, projection ? 5 : 4),
    ) : null,
    React.createElement(Page, { size: 'LETTER', style: styles.page, key: 'disclaimer' },
      ...
      projection ? React.createElement(View, { style: styles.disclaimer },
        React.createElement(Text, { style: combine(styles.disclaimerText, { fontWeight: 'bold', marginBottom: 6, fontStyle: 'normal' }) },
        ...
        React.createElement(Text, { style: combine(styles.disclaimerText, { marginTop: 6 }) },
          'Final pricing should be validated with a licensed actuary, and all stop-loss premiums and TPA fees should be confirmed via formal carrier and vendor quotes before binding.')) : null,
      React.createElement(View, { style: styles.disclaimer },
        React.createElement(Text, { style: combine(styles.disclaimerText, { fontWeight: 'bold', marginBottom: 6, fontStyle: 'normal' }) },
...
function rowEl(label: string, value: string | null | undefined, opts: { bold?: boolean; last?: boolean } = {}) {
  ...
  return React.createElement(View, { style: combine(styles.row, opts.last ? styles.rowLast : null) },
    React.createElement(Text, { style: styles.rowLabel }, label),
    React.createElement(Text, { style: combine(styles.rowValue, opts.bold ? { fontWeight: 'bold' } : null) }, displayValue));
}

function summaryCardEl(label: string, value: string, sub: string, highlight: boolean = false) {
  return React.createElement(View, { style: combine(styles.summaryCard, highlight ? styles.summaryCardHighlight : null) },
    React.createElement(Text, { style: combine(styles.summaryCardLabel, highlight ? styles.summaryCardLabelHighlight : null) }, label),
    React.createElement(Text, { style: combine(styles.summaryCardValue, highlight ? styles.summaryCardValueHighlight : null) }, value),
    React.createElement(Text, { style: combine(styles.summaryCardSub, highlight ? styles.summaryCardSubHighlight : null) }, sub));
}
...
function spendingAccountLabel(s: string): string {
  const map: Record<string, string> = { hsa: 'HSA', hra: 'HRA', fsa: 'FSA', hsa_fsa: 'HSA + Limited FSA', hra_fsa: 'HRA + FSA' };
  return map[s] || s;
}
function hasCarveouts(c: any): boolean {
  return !!(c.dentalEnabled || c.visionEnabled || c.eapEnabled || c.lifeEnabled ||
