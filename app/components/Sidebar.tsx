...
type SidebarProps = {
  active: 'dashboard' | 'find-plans' | 'household' | 'claims' | 'uploaded-files' | 'settings' | 'help' | 'billing' | 'my-plans' | 'compare-plans' | 'employer-benefits';
  firstName: string;
  lastName: string;
  role: string;
  onLogout: () => void;
...
export default function Sidebar({ active, firstName, lastName, role, onLogout }: SidebarProps) {
  ...
  return (
    <aside className="dash-sidebar">
      ...
      <Link href="/household" style={linkReset}>
        <div className={`dash-nav-item ${active === 'household' ? 'active' : ''}`}>
          <div className="dash-nav-icon">👨‍👩‍👧</div> Household
        </div>
      </Link>
      ...
      <Link href="/employer-benefits" style={linkReset}>
        <div className={`dash-nav-item ${active === 'employer-benefits' ? 'active' : ''}`}>
          <div className="dash-nav-icon">🏢</div> Employer Benefits
        </div>
      </Link>
      <div className="dash-section-label">My Data</div>
      <Link href="/claims" style={linkReset}>
        <div className={`dash-nav-item ${active === 'claims' ? 'active' : ''}`}>
          <div className="dash-nav-icon">📄</div> Claims
        </div>
      </Link>
